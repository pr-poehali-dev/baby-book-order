...
Размеры берутся из таблицы products по product_id.
Если product_id не передан — используются дефолтные значения.
"""
import json
import base64
import io
...
def handler(event: dict, context) -> dict:
    ...
    # Размеры: из продукта или дефолт
    if product_id:
        cur.execute(
            "SELECT cover_mm_w, cover_mm_h, spread_mm_w, spread_mm_h, dpi FROM products WHERE id = %s",
            (int(product_id),)
        )
        p = cur.fetchone()
    else:
    ...
    if p:
        cover_w, cover_h, spread_w, spread_h, dpi = p
    else:
        cover_w, cover_h, spread_w, spread_h, dpi = 468, 246, 406, 203, 300

    cover_size  = (mm_to_px(cover_w, dpi),  mm_to_px(cover_h, dpi))
    spread_size = (mm_to_px(spread_w, dpi), mm_to_px(spread_h, dpi))
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        for i, url in enumerate(urls):
            ...
            img_resized.save(img_bytes, format='JPEG', quality=95, dpi=(dpi, dpi))
