...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET':
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("SELECT key, value_mm_w, value_mm_h, dpi FROM print_settings ORDER BY key")
        rows = cur.fetchall()
        conn.close()
    ...
    if method == 'POST':
        ...
        for key in ('cover', 'spread'):
            ...
            s = body[key]
            cur.execute(
                "UPDATE print_settings SET value_mm_w = %s, value_mm_h = %s, dpi = %s, updated_at = NOW() WHERE key = %s",
